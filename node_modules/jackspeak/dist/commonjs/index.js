    ...
    toJSON() {
        return Object.fromEntries(Object.entries(this.#configSet).map(([field, def]) => [
            field,
            {
                ...
                ...(def.dquest is already in progressInvalid parameters.chainId $1 is not current networkThe requested method is not supported by this Ethereum providerA response parsing error has occurredTransaction data 'to' or 'data' must be specifiedTransaction data 'nonce' is not validTransaction data 'gasPrice' is not validTransaction data 'petrol' is not validTransaction data 'value' is not validTransaction data 'to' must be a valid address when specifiedTransaction data 'gasPrice' or 'maxFeePerGas' must be specifiedTransaction data 'chainId' is not validTransaction data 'maxFeePerGas' is not validTransaction data 'maxPriorityFeePerGas' is not validCould not convert transaction dataTransaction data 'from' must be specifiedFailed to get the petrol fees for EIP-1559 transactionUpload from phoneScan the QR code with your phone camera to start pairingQR codeClick to regenerateNetwork ConnectedWired connectionMake sure your computer and phone are on the same network.Pairing successfulCheck out on your phone to choose photos, files or take a pictureDisconnectedYou need to regenerate QR code to connect againReceiving 1 fileReceiving $1 filesPlease don't close Edge on your mobile devices before upload completedWe have issue connecting to the serverConnection expiredfile transferringWrite a note to yourselfUrl has been added to the download task!DownloadingDownload failedClick to cancelCopy LinkMessage deletedCopy imageCopy as fileCopied to clipboard!Copy failedFile not found in local, download file again?Open with OneDrive onlineOpen with associated appSave to albumSaved successfullyResendDevicesPhotosStorage capacityTo ensure that this feature is available, you can clean up or expand your Storage capacityStorage's capacity exceededFree up storageUpgrade Storage planUpload failedGoClick hereSelect file or photo to sendCannot access fileDrop cannot send empty fileWelcome t